	...
	s = function (input) 
	{
	  x = c(10, 17, 21, 26, 29, 32, 36, 38, 50, 63, 64, 67, 68, 
	        ...
	        140, 152, 161, 167, 177, 178, 179, 180, 198, 200, 214, 
	        217, 218, 219, 235, 247, 249, 256, 261, 263, 269)
	  sqlf = paste("SELECT distinct box,", shQuote(input$date, 
	                                               type = "sh"), "refDate  from NESTS where box in (", paste(x, 
	                                                                                                         collapse = ","), ")")
    ...
    d[which(d$nestType == "p" & d$daysSinceFirstCol >= 8 & d$maxClutch < 8), "col"] = "blue"
	  d[which(d$nestType == "s" & d$maxClutch %in% seq(1, 15, 
	                                                   by = 2)), "col"] = "blue"
