...
column(3,
	...
	div(class="row", p(""),
	  div(class = "span4", 
		dateInput('date', 
			label = bib:::labHTML('calendar', 'Date:', 'Reference date.'),
   ...
   selectInput("tabNamHelp", 
               label =  bib:::labHTML('info', 'DATA ENTRY HELP', 'Data entry help for each table and column!'),
  ...
  includeMarkdown(system.file('UI', 'txt', 'block2.md', package = 'bib') )
	...
	div(class="span3", 
		selectInput("mapType", 
			label = bib:::labHTML('globe', 'Map type:', 'Map type:interactive or a simple base map with no decorations.'), 
	...
	div(class="span3", 
	    radioButtons("marks", 
	                 label = bib:::labHTML('compass', 'Markers', 'Add user defined markers to the current map.'),
...
list( 
	...
	div(class="row", p(" ") ,
		div(class="span5", sliderInput("textCex",
						label = bib:::labHTML('gears', 'Text', 'Size of the text on screen map, it will also affect the pdf maps.'), 
						min = 0.5, max = 1.5, value = 0.8, step = 0.05) ), 
		div(class="span5", sliderInput("boxCex", 
						label = bib:::labHTML('gears', 'Box size', 'Box size on screen map, it will also affect the pdf maps.'), 
						min = 0.5, max = 3, value = 2, step = 0.25) ) ),
	#transparency					
	div(class="row", p(" ") ,
		div(class="span5", sliderInput("transp", 
						label = bib:::labHTML('gears', 'Transparency', 'Box transparency on screen map, it will also affect the pdf maps.'), 
	...
	div(class="row", p(" "),
		...
		div(class = "span3",
			selectInput("safeHatchCheck", 
				label = bib:::labHTML('wrench', 'Hatch check', 'How many days in advance to check for hatching. 0 selects the predicted hatching date') , 
				choices = 0:-3, selected =  -3), 
				hr(),
			checkboxInput("hatchNow", 
				label = bib:::labHTML('wrench', 'Hatching NOW:', 'Emphasise boxes where hatching is imminent'), 
				value = TRUE)), 
		
		#young age
			div(class="span3", radioButtons("youngAgeYN", 
			label = bib:::labHTML('wrench', 'Young age', 'Check SELECT to select nests with particular young ages.'),
			choices = c("ALL", "SELECT") , selected = "ALL" ), 
	
	conditionalPanel(condition = "input.youngAgeYN == 'SELECT'",		
		selectInput("youngAge", 
			label = bib:::labHTML('wrench', 'Age on map', 'Select particular young ages <hr> HOLD CTR or SHIFT TO SELECT MULTIPLE VALUES'), 
				choices = 1:25, selected = 14, multiple =  TRUE) ) ), 
				
		# parents
		div(class="span1", radioButtons("parents", 
			label = bib:::labHTML('wrench', 'Parents', 'Show caught parents on the curent map.'),
			choices = c("YES", "NO") , selected = "NO" ) )
	)	
	)
	#  active map end <<<<<<<<<<<<<<<<<
	...
	# experiments starts >>>>>>>>>>>>
		selectInput("experiments", 
						label = bib:::labHTML('bullseye', 'Experiment ID', 'Experiment ID number (see |EXPERIMENTS| table. <hr> Displays experiment on the map and returns data associated with the selected experiment.'),
						choices  = 1:10, 
								selected = 2, 
					  multiple =  FALSE)
